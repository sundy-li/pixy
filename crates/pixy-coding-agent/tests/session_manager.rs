...
fn assistant_message_with_stop_reason(
    ...
    Message::Assistant {
        content: vec![pixy_ai::AssistantContentBlock::Text {
            text: text.to_string(),
            text_signature: None,
        }],
...
fn session_manager_branch_changes_leaf_and_context_path() {
    ...
    match &context.messages[1] {
        Message::Assistant { content, .. } => match &content[0] {
            pixy_ai::AssistantContentBlock::Text { text, .. } => assert_eq!(text, "branch-1"),
        ...
        Message::User { content, .. } => match content {
            UserContent::Blocks(blocks) => {
                let text = blocks
                    .iter()
                    .filter_map(|block| {
                        if let pixy_ai::UserContentBlock::Text { text, .. } = block {
                            Some(text.clone())
                        } else {
                            None
                        }
...
fn session_manager_compaction_entry_persists_and_builds_compacted_context() {
    ...
    match &context.messages[1] {
        Message::Assistant { content, .. } => match &content[0] {
            pixy_ai::AssistantContentBlock::Text { text, .. } => assert_eq!(text, "m2"),
    ...
    match &context.messages[3] {
        Message::Assistant { content, .. } => match &content[0] {
            pixy_ai::AssistantContentBlock::Text { text, .. } => assert_eq!(text, "m4"),
